...
check('余额块 RPC 失败且无旧数据 → 简短失败信息（只降级余额块）', clientSrc.includes("errorTag('bi-err', 'berr', '余额获取失败。请检查网络和 API Key。'"), true);
...
check('花费块 RPC 失败且无旧数据 → 简短降级提示', clientSrc.includes("errorTag('bi-err', 'usageerr', '花费暂不可用；不会影响对话。'"), true);
