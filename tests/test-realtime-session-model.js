...
check('模型切换在绘制前建立旧值与新值的交叉过渡层', client.includes('React.useLayoutEffect(function ()')
  && client.includes('let lastRenderedModelIdentity = null')
  && client.includes('previous: priorRendered')
  && client.includes('function crossfadeModelGroup(subscription)'))
check('模型交叉过渡同时包含离场与入场动画', client.includes('@keyframes bi-model-leave')
  && client.includes('@keyframes bi-model-enter')
  && client.includes('bi-model-stage-out') && client.includes('bi-model-stage-in'))
check('模型动效尊重减少动态效果偏好', client.includes('@media (prefers-reduced-motion: reduce) { .bi-model-stage-out { display: none; } }'))
check('动效键只随会话/模型变化，余额刷新不会重新播放', client.includes('const modelTransitionKey = activeSessionModel')
  && client.includes("activeSessionModel.sessionId + ':' + activeSessionModel.provider + ':' + activeSessionModel.model"))
