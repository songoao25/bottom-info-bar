...
check('模型切换由已挂载节点的 Web Animations 明确触发', client.includes('const modelNodeRef = React.useRef(null);')
  && client.includes('const animation = node.animate([')
  && client.includes('duration: 220')
  && client.includes("easing: 'cubic-bezier(0.22, 0.8, 0.2, 1)'")
  && client.includes('ref: modelNodeRef'))
check('模型动效尊重减少动态效果偏好', client.includes("window.matchMedia('(prefers-reduced-motion: reduce)').matches")
  && client.includes('animation.cancel();'))
