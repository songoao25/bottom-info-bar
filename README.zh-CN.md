...
- **ChatGPT 订阅额度显示**：只读 `~/.codex/auth.json` 中的令牌查询 ChatGPT Plus / Pro 订阅额度（5小时 / 周 / 月 三窗口），显示**剩余百分比**与距重置倒计时。**绑定 / 令牌续期 / ChatGPT 模型路由不在本插件内**——请安装配套插件 [**dsh-chatgpt-subscription**](https://github.com/songoao25)（独立仓库）绑定 ChatGPT 账号；本插件只读令牌显示额度。令牌缺失/失效时信息栏显示「未绑定 / 重新绑定」引导，不报错
