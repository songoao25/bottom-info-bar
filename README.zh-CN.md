...
- **订阅额度显示（ChatGPT & OpenCode Go）**：当前服务商为订阅制时，信息栏显示**订阅服务 + 模型**（如 `OpenCode Go · V4 Flash`）、**5小时 / 周 / 月** 三窗口**剩余额度**（剩余 = 100 − 已用，数值加粗并带颜色：**剩余 >20% 绿色**，**剩余 ≤20% 琥珀色**），以及**距重置倒计时**（最紧窗口，天级格式如 `距重置 1d 21h`）。简洁模式只显示 5 小时窗口，完整模式显示全部三个窗口。任一窗口剩余 ≤20% 时显示 ⚠ 预警。额度来自**两个订阅数据源**：
