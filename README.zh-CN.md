...
每一行均为同一账户的**完整模式**（左）与**简洁模式**（右）：

- **ChatGPT 订阅账户**：展示订阅服务、当前模型、周额度剩余与重置倒计时。
- **DeepSeek API 接入**：展示模型、实时余额、空闲价状态、下次价格切换倒计时和本对话花费。
- **OpenCode Go 订阅账户**：完整模式展示订阅服务、当前模型及 5 小时 / 周 / 月额度；简洁模式展示最近一个重置窗口。
