...
- **订阅额度显示（ChatGPT & OpenCode Go）**：当前服务商为订阅制时，信息栏显示**订阅服务 + 模型**（如 `OpenCode Go · V4 Flash`）、**5小时 / 周 / 月** 三窗口**剩余额度**（剩余 = 100 − 已用，数值加粗，如 `5h 91% · 周 38% · 月 60%`）、**距重置倒计时**（最紧窗口，天级格式如 `距重置 1d 21h`），任一窗口已用 ≥90%（剩余 ≤10%）红色 ⚠ 预警。额度来自**两个订阅数据源**：
  - **ChatGPT / Codex**：只读 `~/.codex/auth.json` 令牌查询 ChatGPT Plus / Pro 订阅额度（5小时 / 周 / 月 三窗口）。**绑定 / 令牌续期 / ChatGPT 模型路由不在本插件内**——请安装配套插件 [**dsh-chatgpt-subscription**](https://github.com/songoao25)（独立仓库）绑定 ChatGPT 账号；本插件只读令牌显示额度。令牌缺失/失效时显示「未绑定 / 重新绑定」引导，不报错
