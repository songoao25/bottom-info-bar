...
function installStyles() {
  ...
  style.textContent = `
      ...
      /* 节点级动画由组件在模型键变化时显式启动；避免 CSS 重建时机影响播放。 */
      .bi-model-transition { will-change: opacity, transform; }
      .bi-model-transition:not(.bi-model-group) { display: inline-block; }
    `;
  ...
  async apply(ctx) {
    ...
    function BottomInfoBar(props) {
      ...
      const modelTransitionKey = activeSessionModel
        ...
        : 'fallback';
      const modelNodeRef = React.useRef(null);
      const previousModelTransitionKeyRef = React.useRef(null);
      React.useEffect(function () {
        const node = modelNodeRef.current;
        if (!node || !activeSessionModel) return undefined;
        const previousKey = previousModelTransitionKeyRef.current;
        previousModelTransitionKeyRef.current = modelTransitionKey;
        // This is an accessibility preference, not an afterthought: users who
        // ask for reduced motion receive the already-updated model directly.
        if (previousKey === null || previousKey === modelTransitionKey
            || (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches)
            || typeof node.animate !== 'function') return undefined;
        // Web Animations is explicit and reliable in a slot-managed React tree;
        // it runs only after the new model DOM node is present.
        ...
        return function () { animation.cancel(); };
      }, [activeSessionModel, modelTransitionKey]);
      ...
      function providerGroup() {
        ...
        if (pr && pr.acceptsImageInput === true) {
          return React.createElement('span', { key: 'prov:' + modelTransitionKey, ref: modelNodeRef, className: 'bi-model-group bi-model-transition', title: provTitle },
        ...
        if (redundant) {
          return React.createElement('span', { key: 'prov:' + modelTransitionKey, ref: modelNodeRef, className: 'bi-model-transition', title: provTitle }, modelLabel);
        }
        return React.createElement('span', { key: 'prov:' + modelTransitionKey, ref: modelNodeRef, className: 'bi-model-transition', title: provTitle },
      ...
      function subscriptionProviderGroup() {
        ...
        return React.createElement('span', { key: 'subprov:' + modelTransitionKey, ref: modelNodeRef, className: 'bi-model-group bi-model-transition', title: title },
